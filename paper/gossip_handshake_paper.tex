...
\title{Gossip Handshake: A Routing-Based Alternative to LoRA\\
Weight-Space Merging for Decentralised Knowledge Sharing}
...
systematically under heterogeneous domain conditions}, producing near-zero keyword recall across all domains.
...
\begin{enumerate}
    \item We provide the first controlled comparison of weight-space merging
          ...
          domains, quantified with keyword-recall scoring, and extend it
          with an overlap ablation showing robustness under $\sim$30\%
          shared vocabulary.
...
veterinary; ``maize'', ``pest'', ``soil'' for agronomy).
The keyword lists are derived from training-domain terminology but
not from test prompts directly, preventing trivial string-matching
leakage.
The domain with the most keyword matches is selected:
...
Because the injected facts contain specific numerical constants,
chemical concentrations, and named biological agents not present in
pretraining data, keyword recall functions as a proxy for
\emph{factual parameter retention} rather than stylistic similarity.
...
or destroy it?''
Because all methods operate under identical data constraints, the
experiment isolates structural composition effects independent of
data scale.
We hypothesise that larger datasets would increase adapter vector
magnitude and potentially \emph{exacerbate} interference during
merging, though confirming this remains future work.
...
across $K=5$ domains ($\%$, mean $\pm$ s.d.).
...
\toprule
\textbf{Config.} & \textbf{Agro} & \textbf{Vet} &
\textbf{Irrig} & \textbf{Soil} & \textbf{Aqua} &
\textbf{Overall} \\
\midrule
...
\textbf{Gossip--KW} & $\mathbf{18.7{\pm}11.5}$ &
...
scoring alone.
This behaviour was observed consistently across all four density
settings ($d \in \{0.3, 0.5, 0.7, 0.9\}$) and did not occur in any
of the five specialist adapter outputs, ruling out a
model-specific or prompt-specific artefact.
...
\begin{table}[htbp]
\centering
\caption{Complete comparison across all methods ($K=5$, $\%$).
The Gossip Handshake Protocol achieves $13\times$ the
performance of the best merge configuration.
$^\dagger$DARE-TIES from pilot ($K=2$); not directly comparable.}
\label{tab:summary}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{l c c c c c c}
\toprule
...
\quad Naive Avg.\ &  0.0 &  0.0 &  0.0 &  4.0 & 0.0 & 0.8 \\
\quad DARE-TIES$^\dagger$ & 20.0 & 20.0 & --- & --- & --- & 20.0 \\
\midrule
\emph{Specialist Adapters} \\
...
\midrule
\emph{Gossip Handshake} \\
\quad \textbf{Gossip--KW} & \textbf{18.7} & \textbf{76.0} &
      \textbf{96.0} & \textbf{85.3} & \textbf{100.0} & \textbf{75.2} \\
\quad Gossip--Cos.\  & 12.0 & 56.0 & 24.0 & 84.0 & 100.0 & 55.2 \\
\bottomrule
\end{tabular}}
\end{table}
...
This asymmetry suggests that fixed-rank LoRA ($r = 16$) may
under-parameterise domain injection at larger scales, leading to
partial retention rather than full factual overwrite.
Higher ranks or domain-adaptive rank allocation~\cite{zhang2023adalora}
may be needed to achieve deeper adaptation at larger model scales.
...
\subsection{Domain Overlap Ablation}
\label{sec:overlap-ablation}

A natural objection to our disjoint-domain results is that routing
trivially outperforms merging when the domains share no vocabulary:
the router's classification task is easy, and the merge suffers maximal
interference from orthogonal update directions. To test whether our
findings hold under realistic domain overlap, we construct a second
$K=5$ configuration in which two domains share approximately 30\% of
their keyword vocabulary.

\textbf{Overlapping domains.}
We replace agronomy and soil science with two new domains:
\emph{agroecology} ($\mathcal{D}_{AE}$, 20 training examples covering
intercropping, agroforestry, allelopathic interactions, and cover
cropping) and \emph{soil restoration} ($\mathcal{D}_{SR}$, 20 examples
covering biochar remediation, vermicompost applications, mycorrhizal
inoculation, and phytoremediation). These domains deliberately share
8 keywords (``organic matter'', ``compost'', ``biochar'',
``mycorrhizal'', ``nitrogen'', ``humic acid'', ``mulch'', and
``soil pH''), creating genuine ambiguity for the router.
The remaining three domains (veterinary, irrigation, aquaculture)
are reused unchanged.

\textbf{Results.}
Table~\ref{tab:overlap-summary} compares routing and merging across
the disjoint and overlapping configurations at both model scales.

\begin{table}[htbp]
\centering
\caption{Routing vs.\ merging under disjoint and overlapping domain
configurations. Gossip--Keyword denotes the keyword router; TIES Best
is the highest-scoring density. Routing accuracy drops from 100\% to
92\% with overlap due to shared vocabulary, but routing still
outperforms merging in all configurations.}
\label{tab:overlap-summary}
\begin{tabular}{l l c c c}
\toprule
\textbf{Scale} & \textbf{Config} & \textbf{Gossip} & \textbf{TIES Best}
  & \textbf{Ratio} \\
\midrule
0.5B & Disjoint & 75.2 &  5.6 & $13.4\times$ \\
0.5B & Overlap  & 65.6 & 19.2 & $3.4\times$ \\
\midrule
1.5B & Disjoint & 37.9 & 19.9 & $1.9\times$ \\
1.5B & Overlap  & 35.5 & 26.4 & $1.3\times$ \\
\bottomrule
\end{tabular}
\end{table}

Three trends emerge. First, \emph{routing accuracy degrades gracefully}:
the keyword router drops from 100\% to 92\% due to the 8 shared
keywords, while the cosine router drops from 80\% to 72\% at 0.5B but
\emph{improves} from 100\% to 96\% at 1.5B---the larger model's
embedding space better separates overlapping domains despite shared
surface vocabulary.

Second, \emph{merging improves with overlap}: TIES scores rise from
$5.6\%$ to $19.2\%$ (0.5B) and $19.9\%$ to $26.4\%$ (1.5B).
This is expected: shared vocabulary means the adapters' update vectors
are less orthogonal, reducing destructive interference during averaging.

Third, and most importantly, \emph{routing still outperforms merging in
every configuration}: a $3.4\times$ advantage at 0.5B and $1.3\times$
at 1.5B. The absolute Gossip scores (65.6\% and 35.5\%) remain
substantially above all merge variants. Naive averaging scores $0.0\%$
at 0.5B (vs.\ $0.8\%$ in the disjoint case) and $23.2\%$ at 1.5B
(vs.\ $8.0\%$), confirming that the overlap benefits merging but
cannot close the gap to routing.

These results refute the charge that our disjoint-domain design
artificially inflates the routing advantage. Even when domains share
nearly a third of their vocabulary, creating genuine router
ambiguity, the Gossip Handshake Protocol consistently preserves more
specialist knowledge than any merge variant tested.
...
different regions of the model's representational capacity), their
sum does not produce a coherent low-rank synthesis.
Instead, it increases the effective rank while destroying the
structured alignment each adapter established with the base model.

More formally, consider the principal angles between the subspaces
$\mathrm{col}(A_i)$ and $\mathrm{col}(A_j)$ for $i \neq j$.
If these angles are large, projections of $A_i B_i^\top$ onto
$\mathrm{col}(A_j)$ are small.
In this regime,
\[
\left\| \sum_{k=1}^{K} A_k B_k^\top \right\|
\]
fails to preserve the directional geometry of any individual
$\Delta\theta_k$.
The averaging operator implicitly assumes partial alignment
between update subspaces.
When that assumption fails, superposition produces destructive
interference rather than compositional knowledge.

The degradation trend observed empirically
($K=2 \rightarrow K=3 \rightarrow K=5$)
is consistent with this interpretation.
As additional approximately orthogonal updates are introduced,
the merged perturbation approaches a high-variance mixture whose
signal-to-noise ratio with respect to any single domain decreases
with $K$.

Inference-time routing avoids this geometric conflict entirely.
By activating exactly one low-rank update per query, the
Gossip Handshake Protocol preserves the integrity of each
specialist subspace.
Selection, rather than averaging, becomes the composition operator.
...
Our primary domains violate this condition maximally: different
vocabulary, different reasoning patterns, different factual
associations, and zero distributional overlap.
Our overlap ablation (\Cref{sec:overlap-ablation}) introduces
$\sim$30\% shared vocabulary, partially satisfying the common-direction
condition. As predicted, merging performance improves (TIES rises
from $5.6\%$ to $19.2\%$ at 0.5B), but routing still outperforms
it by $3.4\times$, suggesting that partial overlap is insufficient
to make merging competitive.
In such settings, routing is not a heuristic convenience but a
structurally necessary composition mechanism.
When knowledge domains occupy orthogonal or even partially shared
regions of parameter space, preservation requires selection, not
averaging.
...
First, the datasets are intentionally small and synthetic
(10--20 training examples per domain).
This design isolates structural composition effects,
but does not measure large-scale generalisation.
Future work should evaluate routing vs.\ merging on
substantially larger, real-world domain datasets.

Second, all experiments use fixed-rank LoRA ($r = 16$).
Higher ranks or adaptive-rank methods may alter the
interference profile during merging.
Systematic rank-scaling experiments are a natural extension.

Third, we evaluate post-hoc routing without joint training.
Learned routing mechanisms, such as Mixture-of-Experts
architectures, may achieve stronger performance under overlap,
though they require centralised coordination and additional
training resources.

Finally, our decentralised framing abstracts away
network-level constraints (bandwidth, latency,
peer discovery).
While orthogonal to the core routing-vs.-merging question,
practical deployments would require engineering evaluation
of these systems-level considerations.

% ==================================================================
\section{Conclusion}
\label{sec:conclusion}

We evaluate weight-space merging and inference-time routing
as competing composition strategies for decentralised
LoRA knowledge sharing.

Across five heterogeneous domains and two model scales,
TIES-Merging consistently fails to preserve specialised
knowledge, with performance degrading further as the
number of merged adapters increases.
Routing via the Gossip Handshake Protocol, by contrast,
retains 88--100\% of specialist performance at 0.5B
and outperforms merging by $1.3\times$--$13\times$ across all tested
configurations.

Our results suggest a broader principle:
when domain adapters encode knowledge in approximately
orthogonal parameter subspaces,
averaging is structurally unstable,
whereas selection is robust.

In decentralised expert networks (where communities
fine-tune independently and share adapters without
coordination), inference-time routing may be a more
reliable composition operator than weight-space merging.

We release all code, data, and logs to encourage
reproduction and extension at larger scales.
