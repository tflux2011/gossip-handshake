...
$\text{col}(A_i)$ and $\text{col}(A_j)$ for $i \neq j$.
If these angles are large, the projections of $A_i B_i^\top$
onto $\text{col}(A_j)$ are small.
In this regime,
\[
...
between update subspaces; when that assumption fails,
superposition produces destructive interference rather than
compositional knowledge.

Crucially, this phenomenon worsens with $K$.
As additional orthogonal adapters are introduced,
the merged update approaches a high-variance mixture whose
signal-to-noise ratio with respect to any single domain
decreases approximately as $1/K$.
This aligns with our empirical observation that merge performance
degrades monotonically from $K=2$ to $K=3$ to $K=5$.

In contrast, inference-time routing avoids superposition entirely.
By activating a single low-rank update per query, the Gossip
Handshake Protocol preserves the geometric integrity of each
specialist subspace.
Switching, rather than averaging, maintains alignment between
domain-specific perturbations and the base model's internal
representations.
