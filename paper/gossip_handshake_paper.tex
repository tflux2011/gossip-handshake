...
More formally, if $\text{col}(A_i)$ and $\text{col}(A_j)$ have low
principal-angle similarity for $i \neq j$, then
\[
\left\| \sum_{k=1}^{K} A_k B_k^\top \right\|
\]
does not preserve the directional geometry of any individual
$\Delta\theta_k$.
The averaging operator implicitly assumes partial alignment
between update subspaces.
When this assumption fails, superposition produces destructive
interference rather than compositional knowledge.

In short, averaging is the wrong operator when the knowledge vectors
