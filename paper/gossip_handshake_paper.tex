...
In other words, merging implicitly assumes that adapter updates
occupy compatible subspaces of parameter space.
When this assumption fails, averaging does not ``combine''
knowledge---it superposes incompatible perturbations.
The result is a high-rank, low-coherence update that weakens
all specialised associations simultaneously.

Routing avoids this geometric conflict entirely.
By activating exactly one low-rank update per query,
the model preserves the integrity of each adapter's
learned subspace.
Selection, rather than superposition, becomes the
