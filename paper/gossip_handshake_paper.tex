...
(Qwen2.5-0.5B-Instruct) fine-tuned on five disjoint African
agricultural domains---agronomy (pest management), veterinary science
(livestock health), irrigation engineering, soil science, and
aquaculture---we demonstrate that:
\textbf{(1)}~TIES-Merging produces models with near-zero keyword recall
across \emph{all} tested density parameters ($1.6$--$5.6\%$ overall,
worsening as $K$ increases from 2 to 5);
\textbf{(2)}~the Gossip Handshake Protocol retains $88$--$100\%$ of
specialist performance ($75.2 \pm 2.8\%$ overall) with zero additional
training; and
...
inference time---retains 88--100\% of specialist performance with zero
additional training, achieving $75.2\%$ overall across five disjoint
domains.
