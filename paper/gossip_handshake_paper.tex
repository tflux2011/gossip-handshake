...
In controlled experiments with two base models---a 494M-parameter
Qwen2.5-0.5B-Instruct and a 1.54B-parameter Qwen2.5-1.5B-Instruct---each
fine-tuned on five disjoint African agricultural domains (agronomy,
veterinary science, irrigation engineering, soil science, and
aquaculture), we demonstrate that:
\textbf{(1)}~TIES-Merging produces models with near-zero keyword recall
across \emph{all} tested density parameters ($1.6$--$5.6\%$ at 0.5B;
$13.6$--$19.9\%$ at 1.5B), worsening as $K$ increases from 2 to 5;
\textbf{(2)}~the Gossip Handshake Protocol retains $88$--$100\%$ of
specialist performance ($75.2 \pm 2.8\%$ overall at 0.5B;
$37.9 \pm 1.8\%$ at 1.5B) with zero additional training; and
\textbf{(3)}~a keyword-based router achieves
$100\%$ routing accuracy at both model scales, establishing routing
...
specialist's accuracy ($96.0 / 96.0$), $101.5\%$ of the soil science
specialist's accuracy ($85.3 / 84.0$), and $100\%$ of the aquaculture
...
\midrule
Uninformed Baseline$^*$ & 20.0 & 20.0 & 20.0 & 20.0 & 20.0 & 20.0 \\
\addlinespace[2pt]
\multicolumn{7}{l}{\footnotesize $^*$Upper bound assuming uniform substring distribution; actual expectation is lower.} \\
\midrule
