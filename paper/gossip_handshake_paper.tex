...
\textbf{Overlapping domains.}
Our primary experiments use disjoint domains, but our overlap ablation
(\Cref{sec:overlap-ablation}) demonstrates that routing maintains a
$3.4\times$ (0.5B) and $1.3\times$ (1.5B) advantage even when domains
share $\sim$30\% of their keyword vocabulary. We do not yet evaluate
\emph{continuous} overlap spectra (e.g., 50\% or 70\% shared vocabulary),
nor hybrid routing--merging strategies that merge overlapping subsets
while routing between disjoint groups.
...
TIES-Merging fails systematically on heterogeneous knowledge domains,
...
A domain overlap ablation further shows that the routing advantage
persists even when domains share $\sim$30\% of their vocabulary
($3.4\times$ at 0.5B, $1.3\times$ at 1.5B), refuting the objection
that disjoint-domain design artificially inflates our results.
